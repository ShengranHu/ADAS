...
def get_json_response_from_gpt(
    ...
    system_message = system_message + " Be concise in your thinking. Your thinking response should be exactly 1 sentence." 
    global api_tracker
    api_tracker = api_call_with_rate_limit(api_tracker, args.max_api_calls)
    logging.info(f"API call #{api_tracker['api_call_count']} to Mistral")
    ...
    response = client.chat.completions.create(
        ...
        temperature=temperature, top_p=0.7, max_tokens=1024,  stop=None, response_format={"type": "json_object"}
    )
    content0 = response.choices[0].message.content
    
    word_count = len(content0.split())
    if word_count > 500:
        logging.error(f"Response too long: {word_count} words")
        print(f"Response too long: {word_count} words")
        return {} 
    
    logging.info(f"Response from Mistral: {content0}")
    
    try:
        # we pass the json string to json.loads to convert it to a dictionary
        json_dict = json.loads(content0) 
    except json.JSONDecodeError:
        logging.error(f"Error decoding JSON")
        json_dict = {}
...
def get_json_response_from_gpt_reflect(
    ...
    global api_tracker
    api_tracker = api_call_with_rate_limit(api_tracker, args.max_api_calls)
    logging.info(f"API call #{api_tracker['api_call_count']} to Mistral (reflect)")
    ...
    logging.info(f"Response from Mistral (reflect): {content}")
...
class LLMAgentBase():
    ...
    def __init__(self, output_fields: list, agent_name: str,                 
            role='helpful assistant', model='mistralai/mistral-large-2-instruct', temperature=0.2) -> None:
        
        self.output_fields = output_fields
        print(f"output_fields: {self.output_fields}")
...
def evaluate(args):
    ...
    file_path = os.path.join(args.save_dir, f"{args.expr_name}_run_archive.json")
    eval_file_path = str(os.path.join(args.save_dir, f"{args.expr_name}_run_archive.json")).removesuffix(".json") + "_evaluate.json"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model', type=str, default='mistralai/mistral-large-2-instruct', choices=[])
    ...
    parser.add_argument('--num_puzzles', type=int, default=1, help='Number of puzzles to process')
