...
visualization = visualize_image_using_emoji(input_grid, res, correct_answer, 
                                                        titles=["Input", "Output", "Answer"])

def reset_api_call_count_if_needed():
    global api_call_count, total_api_call_count, last_reset_time
    ...
    if args.max_api_calls is not None and total_api_call_count >= args.max_api_calls:
        logging.info(f"Reached maximum API calls limit of {args.max_api_calls}. Stopping execution.")
        print(f"\nReached maximum API calls limit of {args.max_api_calls}. Stopping execution.")
        sys.exit(0)

def api_call_with_rate_limit():
    global api_call_count, total_api_call_count
    reset_api_call_count_if_needed()
    
    if api_call_count >= 14:
...
def get_json_response_from_gpt(
    ...
    logging.info(f"API call #{api_call_count} to Gemini")
...
def get_json_response_from_gpt_reflect(
    ...
    api_call_with_rate_limit()
    logging.info(f"API call #{api_call_count} to Gemini (reflect)")
    ...
    try:
        ...
        api_call_with_rate_limit()
        logging.info(f"API call #{api_call_count} to Gemini (reflect) fix JSON")
        try:
...
def evaluate_forward_fn(args, forward_str):
    ...
    if SEARCHING_MODE:
        #arc_dir = r"C:\Users\m\ADAS_gemini\dataset\ARC-800-tasks\training"
        arc_dir = os.path.join(os.path.expanduser("~"), "ADAS", "dataset", "ARC-800-tasks", "training")
    else:
        #arc_dir = r"C:\Users\m\ADAS_gemini\dataset\ARC-800-tasks\evaluation"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_puzzles', type=int, default=2, help='Number of puzzles to process')
    parser.add_argument('--max_api_calls', type=int, default=1, help='Maximum number of API calls')
    ...
    if args.log_info:
        logging.info(f"Search completed. Total API calls: {api_call_count}")
   
    # evaluate
    SEARCHING_MODE = False
    print("Starting evaluation \n")
    evaluate(args)
    print(f"Evaluation completed. Total API calls: {total_api_call_count}")
    if args.log_info:
        logging.info(f"Evaluation completed. Total API calls: {total_api_call_count}")
